...
const WixCodeApiFactory = (
	...
	const { platformEnvData, forceEmptySdks, appDefIdToIsMigratedToGetPlatformApi, sdkFactoriesSiteFeatureConfigs, loadNonMainSdksOnlyIfUsed } = bootstrapData
	...
	async function initSdkFactory({ loader, name }: { loader: WixCodeSdkLoader; name: FeatureName }) {
		...
		const featureSiteConfig = sdkFactoriesSiteFeatureConfigs[name] || {}
	...
	return {
		...
		initWixCodeApiForApplication: async (appDefinitionId: string, appEssentials: AppEssentials) => {
			const isAppMigratedToGetPlatformApi = appDefIdToIsMigratedToGetPlatformApi[appDefinitionId]
			const shouldLoadAllSdks = !((loadNonMainSdksOnlyIfUsed && isAppMigratedToGetPlatformApi) || forceEmptySdks)
			const sdksToLoad = shouldLoadAllSdks
				? Object.keys(wixCodeSdkFactories)
				: [wixCodeSdkLoadersNames.window, wixCodeSdkLoadersNames.seo, wixCodeSdkLoadersNames.site, wixCodeSdkLoadersNames.location, wixCodeSdkLoadersNames.telemetry]
