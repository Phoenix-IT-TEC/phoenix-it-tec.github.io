...
const fetchCssInParallel = (
	...
	fetcher: (params: Partial<CssSiteAssetsParams>) => Promise<SiteAssetsResources['css']>
) => {
	...
	const mainCssPromise = fetcher({ featuresToIgnore: featuresToRunInIsolation.join(',') }).then(
		getFetcherReturnValue('css')
	)
	const isolatedFeaturesPromises = featuresToRunInIsolation.map((feature) =>
		fetcher({ featuresToRun: feature }).then(getFetcherReturnValue(feature))
	)
...
export const PageMainCssFetcher = withDependencies<ICssFetcher>(
	...
	(pageResourceFetcher: IPageResourceFetcher, viewerModel: ViewerModel) => ({
		id: 'css',
		fetch: (pageId, extraModuleParams) => {
			const fetchPromise = pageResourceFetcher.fetchResource(pageId, 'css', extraModuleParams)
