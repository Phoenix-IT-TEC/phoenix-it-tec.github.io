...
const siteMembersApi = (
	...
	const httpClient = createHttpClient({
		...
		...(experiments['specs.thunderbolt.siteMembersMultilingualLanguage']
			? {
					multilingualOptions: {
	...
	const api = {
			...
			return new Promise((resolve, reject) => {
				...
				if (shouldUsePlatformizedAuth) {
					...
					if (forgotPasswordPageId) {
						return popups?.open(forgotPasswordPageId).then(() => {
		...
		async changePassword(newPassword: string, token?: string) {
			...
			const forgotPasswordToken = token ?? featureState.get().resetPasswordToken
		...
		async showNoPermissionsToPageDialog(onCloseCallback?: () => any) {
			if (shouldUsePlatformizedAuth) {
				const noPermissionsPageId = tpaPageIds![TPA_PAGES.NO_PERMISSION]
				if (noPermissionsPageId) {
					router.navigate(TPA_PAGE_IDS_TO_ROUTES[TPA_PAGES.NO_PERMISSION])
					return
				}
			}
			const actions = {
		...
		async showResetPasswordDialog(token: string) {
			...
			if (shouldUsePlatformizedAuth) {
				...
				if (resetPasswordPageId) {
					featureState.update((state) => ({ ...state, resetPasswordToken: token }))
					const url = clearResetPasswordHistory()
					browserWindow?.history.replaceState({}, '', url.href)
					return await router.navigate(TPA_PAGE_IDS_TO_ROUTES[TPA_PAGES.RESET_PASSWORD])
				}
			}
					...
					if (emailConfirmationPageId) {
						const verifyResult: StateMachineResponse = await new Promise<StateMachineResponse>(
							(resolve) => {
								...
								popups?.open(emailConfirmationPageId, closeHandler).then(() => {
									dialogService.hideDialog(true)
								})
							}
						)
