...
const StructureComponent: ComponentType<StructureComponentProps> = React.memo(
	({ id, compId = id, displayedItemId = '', scopeData = emptyScope }) => {
		const { structure: structureStore, comps, disabledComponents, experiments }: RendererProps = useContext(Context)
		...
		if (!experiments['specs.thunderbolt.ooi_lazy_load_components'] && !(compClassType in comps)) {
...
const StructureComponentInner: ComponentType<StructureComponentInnerProps> = ({
	...
	const isSuspenseInStructureComponent = experiments['specs.thunderbolt.ooi_lazy_load_components']
	const Comp: React.ComponentType<any> = compClassType?.startsWith('platform.builder')
		? RemoteStructureRenderer
		: isSuspenseInStructureComponent
		? getComponentToRender(compClassType)
		: comps[compClassType]
	if (isSuspenseInStructureComponent && !Comp) {
		console.warn('Unknown component type', compClassType)
	}
	...
	const children = useCallback(
		...
		[components, displayedItemId, compId, parentScope, propsStore, isDs]
	)
