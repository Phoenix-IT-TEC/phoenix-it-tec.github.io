...
export const Platform = withDependencies(
	...
	(
		...
		function getPlatformEnvData(currentPageId?: string) {
			// TODO: should possibly remove the optional chaining here. It was added due to some failing tests, where the platformEnvData is not available on envApiProvider for some reason.
			return Object.assign({}, ...platformEnvDataProviders.map((envApiProvider) => envApiProvider.platformEnvData?.(currentPageId)))
		}
		const {
		...
		return {
			...
			async appWillLoadPage({ pageId: currentPageId, contextId }) {
				...
				const platformEnvData = getPlatformEnvData(currentPageId)
