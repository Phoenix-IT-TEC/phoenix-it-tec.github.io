...
export default function create(options, dom) {
  ...
  const translateX = startFromOffScreen ? x : `${Math.sin(angleInRad) * distance.value}${unit}`;
  const translateY = startFromOffScreen ? y : `${Math.cos(angleInRad) * distance.value * -1}${unit}`;
  return [{
    ...options,
    easing,
    keyframes: [{
