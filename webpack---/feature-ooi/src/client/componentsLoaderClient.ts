...
async function requireTpaWidgetNativeClient(experiments: Experiments) {
	!experiments['specs.thunderbolt.reactScriptsBeforeApp'] && (await window.externalsRegistry.react.loaded) // wait for React to load since it is loaded dynamically
...
export default withDependencies(
	...
	(
		...
		const getModule = async (widgetId: string, useNoCssComponentIfExists: boolean = true) => {
			...
			const { ooiReactComponentClientWrapper } = await requireTpaWidgetNativeClient(experiments)
			// @ts-ignore
